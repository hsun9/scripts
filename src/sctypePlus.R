...
ScTypeAnnotation <- function(obj, assay, scluster, f_marker_db, tissue, out_path)
{
    ##-- 1. Loding DB
    # load libraries
    ...
    # merge by cluster
    cL_resutls = do.call("rbind", lapply(unique(obj@meta.data[[sclusters]]), function(cl){
        es.max.cl = sort(rowSums(es.max[ ,rownames(obj@meta.data[obj@meta.data[[sclusters]]==cl, ])]), decreasing = !0)
        head(data.frame(cluster = cl, type = names(es.max.cl), scores = es.max.cl, ncells = sum(obj@meta.data[[sclusters]]==cl)), 10)
    }))
    ...
    for(j in unique(sctype_scores$cluster)){
        ...
        obj@meta.data$cell_type[obj@meta.data[[sclusters]] == j] = celltype
    ...
    # v3.1
    obj@meta.data$cluster_plus <- paste0('C', obj@meta.data[[sclusters]], '.', obj@meta.data$cell_type2)
    obj@meta.data$sctype.score <- sctype_scores$scores[match(obj@meta.data[[sclusters]], sctype_scores$cluster)]
...
auto_detect_tissue_type <- function(path_to_db_file, seuratObject, scluster, scaled, assay = "RNA", ...){
  ...
  # get all tissue types in DB
    ...
    cL_resutls = do.call("rbind", lapply(unique(seuratObject@meta.data[[sclusters]]), function(cl){
        es.max.cl = sort(rowSums(es.max[ ,rownames(seuratObject@meta.data[seuratObject@meta.data[[sclusters]]==cl, ])]), decreasing = !0)
