...
AllMotifs_2D <- function(obj=NULL, comp_ct=NULL, label_id=NULL, outdir=NULL)
{   
    metadata <- obj@meta.data[c('cell_type', 'cell_type2')]

    # 1
    motif_sig <- t(obj@assays$chromvar@data)
    ...
    p <- p + geom_text_repel(
                   data = df_label,
                   aes_string(label = rownames(df_label)),
                   size = 3,
                   colour = "#B03A2E",
    ...
    pdf(paste0(outdir, '/allmotif_2d.pdf'), width=2.5, height=2.5, useDingbats=FALSE)
    print(p)
    dev.off()
}
