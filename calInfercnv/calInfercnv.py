...
parser.add_argument('-m', '--mode', default='hpc', help='mode')
...
def main():
    ...
    fconfig = f'{path}/config.yaml'
    config = ReadYaml(fconfig)
    r_script = 'Rscript'
    if 'RSCRIPT' in config[mode]:
        r_script = config[mode]['RSCRIPT']
    ...
    fgene = ''
    if (ref == 'mm10'):
        fgene = config[mode]['MM10_GENES']
    if (ref == 'hg38'):
        fgene = config[mode]['HG38_GENES']
    ...
    fscript = os.path.join(path, '/src/seu2mtx.R')
    cmd = f'{r_script} {fscript} --rds {dy['rds']} --assay {dy['assay']} --ctl "{dy['ctl']}" --form {dy['form']} --outdir {outdir}'
    os.system(cmd)
    ...
    # 3
    print('[INFO] Call CNV ...')
    fscript = os.path.join(path, '/src/run_infercnv.R')
    cmd = f'{r_script} {fscript} --dir {outdir}/mtx --func {dy['cnv_model']}  --ref_group {dy['ref_group']} --sd_amp {dy['sd_amp']} --outdir {outdir}'
    os.system(cmd)
